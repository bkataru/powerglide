/// powerglide × igllama BF16 trial harness
///
/// Runs T01–T13 across the full Qwen3.5 weight-class lineup at BF16 (full
/// precision). Each model is served sequentially on :8090 — the harness
/// manages the igllama lifecycle: kill, spawn, health-poll, run, kill.
///
///   zig build trial-bf16
///
/// GGUFs required (download with `igllama pull`):
...
const BF16Model = struct {
    name: []const u8,
    file: []const u8,
};

const BF16_MODELS = [_]BF16Model{
    .{ .name = "0.8B-BF16", .file = "Qwen3.5-0.8B-BF16.gguf" },
    .{ .name = "2B-BF16",   .file = "Qwen3.5-2B-BF16.gguf" },
    .{ .name = "4B-BF16",   .file = "Qwen3.5-4B-BF16.gguf" },
    .{ .name = "9B-BF16",   .file = "Qwen3.5-9B-BF16.gguf" },
};
...
// Reuses an Endpoint-like struct for runTask compatibility.
const Endpoint = struct { name: []const u8, port: u16, model: []const u8 };
    ...
    while (turn < MAX_TURNS) : (turn += 1) {
        ...
        if (msgs.items.items.len > 2 + 8) {
            ...
            if (total > keep_front + keep_back) {
                ...
                for (msgs.items.items[drop_start..drop_end]) |m| allocator.free(m.content);
                const tail = msgs.items.items[drop_end..];
                for (0..tail.len) |i| msgs.items.items[drop_start + i] = tail[i];
...
/// Kill any running igllama process. Sleeps only if a process was actually killed.
fn killIgllama(allocator: std.mem.Allocator) void {
    const res = std.process.Child.run(.{
        .allocator = allocator,
        .argv      = &[_][]const u8{ "/bin/sh", "-c", "pkill -f 'igllama api'" },
    }) catch return;
    const killed = switch (res.term) { .Exited => |c| c == 0, else => false };
    allocator.free(res.stdout);
    allocator.free(res.stderr);
    // Sleep only if a process was actually killed — give OS time to release port.
    if (killed) std.Thread.sleep(3 * std.time.ns_per_s);
}

/// Spawn igllama serving `model_file` on BF16_PORT. Returns the Child (running).
fn spawnIgllama(allocator: std.mem.Allocator, model_file: []const u8) !std.process.Child {
    const port_str = try std.fmt.allocPrint(allocator, "{d}", .{BF16_PORT});
...
fn waitForHealth(allocator: std.mem.Allocator, port: u16, w: anytype) !bool {
    const deadline = std.time.milliTimestamp() + @as(i64, HEALTH_TIMEOUT_S) * 1000;
    try w.print("  Waiting for health (up to {d}s)...", .{HEALTH_TIMEOUT_S});
    var dots: u32 = 0;
...
pub fn main() !void {
    ...
    try w.writeAll("\npowerglide x igllama — BF16 trial harness\n");
    try w.print("Tasks: T01-T{d}   temp={d:.1} top_p={d:.1} top_k={d}   port={d}\n\n",
        .{ TASKS.len, TEMPERATURE, TOP_P, TOP_K, BF16_PORT });

    var results: [BF16_MODELS.len][TASKS.len]?TaskResult =
        .{.{null} ** TASKS.len} ** BF16_MODELS.len;
    defer {
        for (&results) |*row| for (row) |*mr| if (mr.*) |*r| r.deinit(allocator);
    }

    for (BF16_MODELS, 0..) |m, mi| {
        try w.print("\n{s}\n", .{SEP_BLOCK});
        try w.print("  MODEL: {s}  (:{d})  file={s}\n", .{ m.name, BF16_PORT, m.file });
        try w.print("{s}\n", .{SEP_BLOCK});

        // Kill any running instance, then start this model
        try w.writeAll("  Killing existing igllama...\n");
        killIgllama(allocator);

        try w.print("  Spawning igllama with {s}...\n", .{m.file});
        var child = spawnIgllama(allocator, m.file) catch |err| {
            try w.print("  SPAWN_ERR: {s} — skipping\n", .{@errorName(err)});
            continue;
        };

        const healthy = try waitForHealth(allocator, BF16_PORT, w);
        if (!healthy) {
            _ = child.kill() catch {};
            _ = child.wait() catch {};
            continue;
    ...
    // ── Summary ───────────────────────────────────────────────────────────────
    try w.print("\n\n{s}\n", .{SEP_THICK});
    try w.writeAll("  RESULTS — powerglide x igllama BF16 trial\n");
    try w.print("{s}\n", .{SEP_THICK});
    ...
    for (BF16_MODELS, 0..) |m, mi| {
        ...
        try w.print("\n  {s}: {d}/{d} passed  {d} turns  {d:.0}s\n",
            .{ m.name, passed, TASKS.len, total_turns,
