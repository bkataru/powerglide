...
pub const Config = struct {
    ...
    pub fn save(self: *const Config, allocator: std.mem.Allocator, path: []const u8) !void {
        ...
        try buf.appendSlice("\",");
        try buf.appendSlice("\"api_base_url\":\"");
        try buf.appendSlice(self.api_base_url);
        try buf.appendSlice("\",");
        try buf.appendSlice("\"max_steps\":");
        try buf.writer().print("{},", .{self.max_steps});
        try buf.appendSlice("\"velocity_ms\":");
        try buf.writer().print("{},", .{self.velocity_ms});
        try buf.appendSlice("\"think_budget_ms\":");
        try buf.writer().print("{},", .{self.think_budget_ms});
        try buf.appendSlice("\"shell\":\"");
        try buf.appendSlice(self.shell);
        try buf.appendSlice("\",");
        try buf.appendSlice("\"terminal_pool_size\":");
        try buf.writer().print("{},", .{self.terminal_pool_size});
        try buf.appendSlice("\"max_agents\":");
        try buf.writer().print("{},", .{self.max_agents});
        try buf.appendSlice("\"agent_heartbeat_ms\":");
        try buf.writer().print("{}", .{self.agent_heartbeat_ms});
        try buf.appendSlice("}");
