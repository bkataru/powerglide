...
`0.3.2` — 195/195 tests passing, 0 leaks.
...
27. ✅ 9B T01-T17 re-run — all 5 quant variants measured against full 17-task suite — **17/17 across all quantizations**
28. ✅ igllama v0.3.11 — strip residual `</think>` tokens when `--no-think` is active (PR #85)
29. ✅ /simplify + /security-review pass — OAIResponse dangling pointers fixed, config save JSON fixed, getToolCalls leak fixed, MCP versions synced, unused import removed, SseParser.init signature cleaned
